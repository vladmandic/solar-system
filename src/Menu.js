...
function createCSS() {
  ...
  .menu-button { border: 0; background: ${theme.buttonBackground}; width: -webkit-fill-available; padding: 2px; margin: 8px; cursor: pointer;
  ...
  input[type=datetime-local] { width: -webkit-fill-available; padding: 2px; height: 1.2rem; background: black; color: white; border: none; }
  input[type=datetime-local] + label { width: -webkit-fill-available; padding: 2px; line-height: 1.2rem; padding-left: 12px; }
...
export class Menu {
  ...
  addInputText(title, object, variable, callback) {
    const el = document.createElement('div');
    el.className = 'menu-item';
    el.innerHTML = `<div class="menu-item"><input class="menu-input" type="text" id="${this.newID}" value="${object[variable]}}/><label class="menu-item" for="${this.ID}">${title}</label></div>`;
    if (this.container) this.container.appendChild(el);
    el.addEventListener('change', (evt) => {
      if (evt.target) {
  ...
  addInputNumber(title, object, variable, callback) {
    const el = document.createElement('div');
    el.className = 'menu-item';
    el.innerHTML = `<div class="menu-item"><input class="menu-input" type="number" id="${this.newID}" value="${object[variable]}}/><label class="menu-item" for="${this.ID}">${title}</label></div>`;
    if (this.container) this.container.appendChild(el);
    el.addEventListener('change', (evt) => {
      if (evt.target) {
        object[variable] = parseFloat(evt.target.value);
        if (callback) callback(parseFloat(evt.target.value));
      }
    });
    return el;
  }

  addInputDateTime(title, object, variable, callback) {
    const el = document.createElement('div');
    el.className = 'menu-item';
    const pad = (s) => s.toString().padStart(2, '0');
    const ts = `${object[variable].getFullYear()}-${pad(object[variable].getMonth() + 1)}-${pad(object[variable].getDate())}T${pad(object[variable].getHours())}:${pad(object[variable].getMinutes())}`;
    el.innerHTML = `<div class="menu-item"><input class="menu-input" type="datetime-local" id="menu-input-${title}" value="${ts}"/><label class="menu-item" for="${this.ID}">${title}</label></div>`;
    if (this.container) this.container.appendChild(el);
    el.addEventListener('change', (evt) => {
      if (evt.target) {
        object[variable] = new Date(evt.target.value);
        if (callback) callback(new Date(evt.target.value));
      }
    });
    return el;
  }

  updateInputDateTime(title, date) { // eslint-disable-line class-methods-use-this
    const el = document.getElementById(`menu-input-${title}`);
    const pad = (s) => s.toString().padStart(2, '0');
    const ts = `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}`;
    if (el) el.value = ts;
  }

  addList(title, items, selected, callback) {
    const el = document.createElement('div');
    el.className = 'menu-item';
    let options = '';
  ...
  addChart(title, id, width, height, color) {
